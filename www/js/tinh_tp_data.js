...
let ordered_cities_key = {};

ordered_cities_key = Object.keys(tinh_tp_data).sort((a, b) => {
    const oldCity = tinh_tp_data[a];
    const newCity = tinh_tp_data[b];
    if(oldCity.name < newCity.name) { return -1; }
    if(oldCity.name > newCity.name) { return 1; }
    return 0;
})
